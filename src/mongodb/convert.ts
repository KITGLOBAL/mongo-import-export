...
export function convertExtendedJSON(doc: any): any {
  ...
  if (doc !== null && typeof doc === 'object') {
    if (doc['$oid']) {
      logger.debug(`Преобразование $oid: ${doc['$oid']} в ObjectId`);
      return new ObjectId(doc['$oid']);
    }
    if (doc['$date'] && typeof doc['$date'] === 'object' && doc['$date']['$numberLong']) {
      logger.debug(`Преобразование $date.$numberLong: ${doc['$date']['$numberLong']} в Date`);
      return new Date(Number(doc['$date']['$numberLong']));
    }
    if (doc['$date'] && typeof doc['$date'] === 'string') {
      logger.debug(`Преобразование $date: ${doc['$date']} в Date`);
      return new Date(doc['$date']);
    }
    if (doc['$numberInt']) {
      logger.debug(`Преобразование $numberInt: ${doc['$numberInt']} в Number`);
    ...
    for (const [key, value] of Object.entries(doc)) {
      if ((key === '_id' || key === 'theme_id') && typeof value === 'string' && isValidObjectId(value)) {
        logger.debug(`Преобразование строки ${key}: ${value} в ObjectId`);
        ...
        logger.debug(`Преобразование строки ${key}: ${value} в Date`);
